...
export async function createAttendee(req, res) {
  ...
  try {
    ...
    res.status(201).json({ message: "Attendee created successfully", newAttendee });
  } catch (error) {
    console.log("Error creating new attendee", error);
    res.status(500).json({ error: "Error occured while creating a new attendee" });
  }
}
  ...
  export async function updateAttendee(req, res) {
    ...
    try {
      ...
      res.status(200).json(attendee);
        ...
        res.status(500).json({ error: "Error occured while updating the attendee" });
    }
  }
